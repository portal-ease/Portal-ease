<x-app-layout :portal="$portal">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        @forelse($portal->projects as $project)
            ...
            <a class="bg-white rounded-xl shadow-md p-5 hover:shadow-lg transition" href="{{ route('portal.project.show', ['project' => $project, 'portal' => $portal]) }}?status={{ $status }}">
                <h2 class="text-lg font-semibold text-gray-800 mb-2">{{ $project->name }}</h2>
                <p class="text-sm text-gray-600">Start: {{ \Carbon\Carbon::parse($project->start_date)->format('M d, Y') }}</p>
                <p class="text-sm text-gray-600">End: {{ \Carbon\Carbon::parse($project->end_date)->format('M d, Y') }}</p>
            </a>
        @empty
